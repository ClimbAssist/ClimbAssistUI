export const fetch = {
  methods: {
    async fetchData() {
      //get country data
      try {
      const api = await this.$axios.$get("/v1/countries/", { progress: false });
      let countries = [];
      let countriesList = api.data;
      for (let i in countriesList) {
        let countryRes = await this.$axios.$get("v1/countries/" + api.data[i].countryId + "?depth=6");
      ...
      this.$store.commit("filter/dataSet", true);
      this.$store.commit("filter/updateLoadError", "There was a problem loading the data, please try again later.")
    }
    }
  }
};
