<template>
  <v-container>
    ...
    <v-btn v-if="errorPointsId" @click="deletePoints(errorPointsId)" color="red">
        Delete Line
    </v-btn>
    <v-container>
        <v-layout
...
export default {
  ...
  methods: {
    async debug() {
        ...
        try {
            await this.$axios.$get('/v1/crags/' + this.$route.params.edit + '?depth=4')
            ...
            if (error.response.data.error.type === "InvalidOrderingException") {
                ...
                this.currentTask = "error not associated with ordering, please contact administrator"
            }
        }
    },
    async checkPaths() {
        ...
        try {
            let paths = (await this.$axios.$get('/v1/crags/' + this.$route.params.edit + '/paths')).data;
            if (paths.length > 0) {
                for (let i in paths) {
                    try {
                        await this.$axios.$get('/v1/paths/' + paths[i].pathId + '/path-points?ordered=true');
                        if (i == paths.length - 1) {
                        ...
                        if (error.response.data.error.type == "InvalidOrderingException") {
                            this.currentTask = "ordering error found in " + paths[i].pathId + "delete this path?"
                            this.errorPathId = paths[i].pathId;
                        } else {
                            this.currentTask="error not associated with order, please contact admin."
    ...
    async checkWalls() {
        ...
        try {
            let newCrag = (await this.$axios.$get('/v1/crags/' + this.$route.params.edit + '?depth=1')).data;
            this.status.walls=this.icon.success;
            this.checkRoutes(newCrag)
        } catch (error) {
            this.status.walls=this.icon.error;
            if (error.response.data.error.type === "InvalidOrderingException") {
                this.fixResources(this.$route.params.edit, error.response.data.error.message, "walls", "crags");
            } else {
                this.currentTask="error not associated with order, please contact admin."
                this.debugging = false;
            }
    ...
    async checkRoutes(crag) {
        ...
        try {
            let newCrag = (await this.$axios.$get('/v1/crags/' + this.$route.params.edit + '?depth=2')).data;
            this.status.routes=this.icon.success;
            this.checkPitches(newCrag);
        } catch (error) {
            this.logs.push(error.response.data);
            if (error.response.data.error.type === "InvalidOrderingException") {
                this.currentTask="Error in Routes Order getting unordered routes...";
                this.status.routes=this.icon.error;
                for (let i in crag.walls) {
                    try {
                        await this.$axios.$get('/v1/walls/' + crag.walls[i].wallId + '/routes?ordered=true');
                    } catch (error) {
                        this.logs.push(error.response.data)
                        if (error.response.data.error.type === "InvalidOrderingException") {
                            this.fixResources(crag.walls[i].wallId, error.response.data.error.message, "routes", "walls");
                        } else {
                            this.currentTask="error not associated with order, please contact admin."
                            this.debugging = false;
                        }
    ...
    async checkPitches(crag) {
        ...
        try {
            let newCrag = (await this.$axios.$get('/v1/crags/' + this.$route.params.edit + '?depth=3')).data;
            this.status.pitches=this.icon.success;
            this.checkPoints(newCrag);
        } catch (error) {
            this.logs.push(error.response.data);
            if (error.response.data.error.type === "InvalidOrderingException") {
                this.currentTask="Error in Pitches Order getting unordered pitches...";
                this.status.pitches=this.icon.error;
                for (let i in crag.walls) {
                    for (let ri in crag.walls[i].routes) {
                        try {
                            await this.$axios.$get('/v1/routes/' + crag.walls[i].routes[ri].routeId + '/pitches?ordered=true');
                        } catch (error) {
                            this.logs.push(error.response.data)
                            if (error.response.data.error.type === "InvalidOrderingException") {
                                this.fixResources(crag.walls[i].routes[ri].routeId, error.response.data.error.message, "pitches", "routes");
                            } else {
                                this.currentTask="error not associated with order, please contact admin."
                                this.debugging = false;
                            }
    ...
    async checkPoints(crag) {
        ...
        try {
            let newCrag = (await this.$axios.$get('/v1/crags/' + this.$route.params.edit + '?depth=4')).data;
            ...
            for (let i in crag.walls) {
                for (let ri in crag.walls[i].routes) {
                    for (let pi in crag.walls[i].routes[ri].pitches) {
                        try {
                        await this.$axios.$get('/v1/pitches/' + crag.walls[i].routes[ri].pitches[pi].pitchId + '/points?ordered=true');
                        } catch (error) {
                            this.logs.push(error.response.data)
                            this.status.points=this.icon.error;
                            if (error.response.data.error.type == "InvalidOrderingException") {
                                this.currentTask = "ordering error found in points for" + crag.walls[i].routes[ri].pitches[pi].pitchId + "delete this route line (points)?"
                                this.errorPointsId = crag.walls[i].routes[ri].pitches[pi].pitchId;
                                this.debugging = false;
                            } else {
                                this.currentTask="error not associated with order, please contact admin."
                                this.debugging = false;
    ...
    async fixResources(errorId, error, type, parent) {
        this.currentTask="error in " + type + " order, retrieving walls unordered";
        let resources = (await this.$axios.$get('/v1/' + parent + '/' + errorId + '/' + type)).data;
        let resourceId;
        if (type === "walls") {
            resourceId = "wallId";
        } else if (type === "routes") {
        ...
        this.currentTask="fix the following ordering error in the " + type + ",then run the debugger again"
    ...
    async updateResource(resource, i) {
        ...
        try {
            ...
            this.$store.commit("snackbar/updateTimeout", 10000);
            this.$store.commit("snackbar/updateMessage", "failed to update" + type + error.response.data.error.message);
            this.$store.commit("snackbar/updateSnackbar", true);
            this.$store.commit("snackbar/updateLink", undefined);
            this.$store.commit("snackbar/updateLinkMessage", undefined);
        }
    ...
    async deletePath(pathId) {
        ...
        let points = (await this.$axios.$get('/v1/paths/' + pathId + '/path-points')).data;
        let obj = {
            pathPointIds: []
        }
        for (let i in points) {
            obj.pathPointIds.push(points[i].pathPointId);
        }
        try {
            await this.$axios.$delete('/v1/path-points', {data: obj});
            try {
                await this.$axios.$delete('/v1/paths/' + pathId)
                this.currentTask = "deleted" + pathId + "you will need to recreate this path in the editor. Select run debug again to check the crag."
                this.debugging = false;
                this.errorPathId = undefined;
                return;
            } catch (error) {
    ...
    async deletePoints(pitchId) {
        ...
        let points = (await this.$axios.$get('/v1/pitches/' + pitchId + '/points')).data;
        let obj = {
            pointIds: []
        }
        for (let i in points) {
            obj.pointIds.push(points[i].pointId);
        }
        try {
            await this.$axios.$delete('/v1/points', {data: obj});
            this.currentTask = "deleted route line (points) you will need to recreate this line in the editor. Select run debug again to check the crag."
            this.debugging = false;
            this.errorPointsId = undefined;
        } catch (error) {
