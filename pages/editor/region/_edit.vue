      ...
      <v-container>
        ...
        <v-container>
          ...
          <v-layout align-center justify-end row>
            <v-btn class="mx-2" color="primary" :disabled="formCheck" @click="updateRegion()">
              Update
            </v-btn>
            <v-btn
              color="red"
...
export default {
  ...
  methods: {
    ...
    async updateRegion() {
      try {
        ...
        this.$store.commit("snackbar/updateTimeout", 10000);
        this.$store.commit("snackbar/updateMessage", "failed to update region" + error.response.data.error.message);
        this.$store.commit("snackbar/updateSnackbar", true);
        this.$store.commit("snackbar/updateLink", undefined);
        this.$store.commit("snackbar/updateLinkMessage", undefined);
    ...
    async deleteRegion() {
      try {
        ...
        this.$store.commit("snackbar/updateType", "error");
        this.$store.commit("snackbar/updateTimeout", 10000);
        this.$store.commit("snackbar/updateMessage", "failed to delete region" + error.response.data.error.message);
        this.$store.commit("snackbar/updateSnackbar", true);
        this.$store.commit("snackbar/updateLink", undefined);
        this.$store.commit("snackbar/updateLinkMessage", undefined);
        console.log(error.response.data.error.message);
  ...
  async fetch({ store, params }) {
    ...
    const api = (await axios.get("/v1/regions/" + params.edit + "?depth=1")).data;
